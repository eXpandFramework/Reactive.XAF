...
namespace Xpand.XAF.ModelEditor.Module.Win.MSBuildLocator {
    internal static class DotNetSdkLocationHelper {
        private static readonly Regex DotNetBasePathRegex = new("Base Path:(.*)$", RegexOptions.Multiline);
        private static readonly Regex VersionRegex = new(@"^(\d+)\.(\d+)\.(\d+)", RegexOptions.Multiline);
        private static readonly Regex SdkRegex = new(@"(\S+) \[(.*?)]$", RegexOptions.Multiline);
