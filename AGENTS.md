...
- **Tests** — NUnit + Shouldly + Moq; `dotnet test src\Tests\Tests.sln --settings Build\Tests.runsettings` (120-min session timeout). The Tests.sln builds with `-WarnAsError` in Debug via the psake CompileTests task.
