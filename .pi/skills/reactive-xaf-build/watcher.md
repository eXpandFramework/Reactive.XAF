...
description: The background AzDO chain watcher — polls the publish chain (def 23 → 72 → 89), toasts on every check, asserts the nugets on the eXpand nuget server and the GitHub pre-release, steers on failure.
---

# watcher.ts — background AzDO chain watcher

Companion of `.pi/extensions/reactive-xaf-build/watcher.ts`. Started by
`monitorPhase` (build.ts) after prx, or by `/devexpress watch` for a running
...
- Final step ASSERTS the GitHub PRE-RELEASE for the lab build
  (`api.github.com/repos/eXpandFramework/Reactive.XAF/releases` — tag =
  version, prerelease=true) with RETRIES (`ghRetries` default 6 ×
  `ghRetryMs` default 30 s) to absorb the release-creation race. Found →
  `... succeeded — GitHub pre-release <v> found — chain complete.`; missing
  → warning + steer.
...
advance, nuget assertion, GitHub assertion + race retry, terminal steer,
give-up, replace).
