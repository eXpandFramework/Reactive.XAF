...
namespace Xpand.Extensions.StringExtensions {
    public static partial class StringExtensions {
        ...
        public static string EncloseHTMLTag(this string input, params string[] tags)
            => tags.Select(tag => tag.Remove("<", ">").Trim()).WhereNotNullOrEmpty()
                .Reverse().Aggregate(input, (current, tag) => {
                    var tagParts = tag.Split([' '], 2, StringSplitOptions.RemoveEmptyEntries);
                    var tagName = tagParts[0];
                    var attributes = tagParts.Length > 1 ? " " + tagParts[1].Trim() : "";
