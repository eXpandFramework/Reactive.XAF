...
namespace Xpand.Extensions.Reactive.Channel {
    public static class ReactiveChannel {
        ...
        public static IObservable<Unit> HandleRequestWith<TRequest>(this object o, Func<TRequest, IObservable<Unit>> handler) 
            => o.HandleRequestWith<TRequest, Unit[]>(request => handler(request).Select(_ => Array.Empty<Unit>()));
