        ...
        public static IEnumerable<TestCaseData> ExecutionSelectors() {
            ...
            TestCaseData NewTestData<TEventArg,TAction>(ActionFactory actionFactory,string eventName,ActionConfig actionConfig,ActionRepeat actionRepeat) where TEventArg : EventArgs where  TAction : ActionBase 
                => new TestCaseData(
                    ...
                    actionFactory,actionConfig,actionRepeat
                ).SetName($"{typeof(TAction).Name}_{eventName})");
            TestCaseData NewConcatExecutionData<TEventArg,TAction>(ActionFactory actionFactory,ActionConfig actionConfig,ActionRepeat actionRepeat) where TEventArg : ActionBaseEventArgs where  TAction : ActionBase 
                => new TestCaseData(
                    new ExecutionPipe(source => source.WhenConcatExecution<TEventArg,TAction>(_ => 100.Milliseconds().Timer().SelectMany(_ => Observable.Throw<TAction>(new Exception())))),
                    actionFactory,actionConfig,actionRepeat
            ...
            yield return NewConcatExecutionData<SimpleActionExecuteEventArgs,SimpleAction>(NewSimpleAction(),config,repeatWhenEnabled);
            ...
            yield return NewConcatExecutionData<ParametrizedActionExecuteEventArgs,ParametrizedAction>(NewParametrizedAction(),config,repeatWhenEnabled);
            ...
            yield return NewConcatExecutionData<SingleChoiceActionExecuteEventArgs,SingleChoiceAction>(NewSingleChoiceAction(),config,repeatWhenEnabled);
            ...
            yield return NewConcatExecutionData<PopupWindowShowActionExecuteEventArgs,PopupWindowShowAction>(NewPopupWindowAction(),config,repeatWhenEnabled);
        }
    }
}