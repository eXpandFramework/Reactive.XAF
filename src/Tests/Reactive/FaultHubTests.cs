...
namespace Xpand.XAF.Modules.Reactive.Tests{
    ...
    public class FaultHubTests{
        ...
        public void Exceptions_emitted_from_FaultHub(){
            using var observer = Observable.Throw<Unit>(new Exception()).ToResilient() .Test();
        ...
        public async Task PreBus_Emits_Exceptions_before_Bus(){
            ...
            await error.CompleteOnError().FirstOrDefaultAsync();
        ...
        public void Any_UpStream_emits_until_completion_when_inner_resilient_stream_throws(){
            ...
            testObserver.CompletionCount.ShouldBe(1);
            count.ShouldBe(3);
            _busObserver.ItemCount.ShouldBe(3);
        }
        ...
        [Test]
        public void Inner_stream_can_CompleteOnError_when_error_when_outer_stream_resilient(){
            var bus = Unit.Default.Observe()
                .SelectManyResilient(_ => Observable.Defer(() => Observable.Defer(() => Observable.Throw<Unit>(new InvalidOperationException())
                    .CompleteOnError(match:e => e is InvalidOperationException).WhenCompleted()))
                     );
