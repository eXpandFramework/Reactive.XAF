...
namespace ALL.Win.Tests{
    ...
    public class AllWinTests : BaseTest{
...
#if !NETCOREAPP3_1
            var fileName = Path.GetFullPath($@"{AppDomain.CurrentDomain.ApplicationPath()}\..\TestWinApplication\TestApplication.Win.exe");
#else
            var fileName = Path.GetFullPath($@"{AppDomain.CurrentDomain.ApplicationPath()}\..\..\TestWinDesktopApplication\TestApplication.WinDesktop.exe");
#endif
        ...
        public async Task Win_EasyTest_InLocalDb(){
            var connectionString = $"Integrated Security=SSPI;Pooling=false;Data Source=(localdb)\\mssqllocaldb;Initial Catalog=TestApplicationWin{AppDomain.CurrentDomain.UseNetFramework()}";
