...
namespace ALL.Tests {
	public static class OfficeCloudService {
		...
		private static async Task CheckOperation(this ICommandAdapter commandAdapter, MapAction mapAction) {
			await commandAdapter.Execute(() => {
				...
				commandAdapter.Execute(
					new CheckDetailViewCommand<DevExpress.Persistent.BaseImpl.Task>((task => task.Description,
						mapAction.ToString())));
			});
		}
		...
		public static async Task TestOfficeCloudService(this ICommandAdapter commandAdapter,
			string navigationItemCaption, string editorName) {
			commandAdapter.Execute(new NavigateCommand(navigationItemCaption), new ActionCommand(Actions.New));
			commandAdapter.Execute(new FillObjectViewCommand((editorName, "New")), new WaitCommand(2000),
		...
		private static void Disconnect(this ICommandAdapter commandAdapter, string signOutCaption,
			string signInCaption) {
		...
		public static IObservable<Unit> Authenticate(this ICommandAdapter commandAdapter, string signInCaption,
			string passFileName, string email, Action afterSingInActionExecuted = null) {
			if (commandAdapter.GetTestApplication().Platform() != Platform.Win) {
				Observable.Start(() => commandAdapter.Execute(new ActionCommand(signInCaption)))
					.Timeout(TimeSpan.FromSeconds(Debugger.IsAttached ? 10 : 30))
					.OnErrorResumeNext(Observable.Empty<Unit>().FirstOrDefaultAsync()).Wait();
			}
			else {
			...
			commandAdapter.Execute(new SendKeysCommand(Win32Constants.VirtualKeys.Return),
				new WaitCommand((int)(WaitInterval * 1.5)));
			Win32Declares.WindowFocus.SetForegroundWindow(foregroundWindow);
