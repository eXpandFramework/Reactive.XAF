...
namespace Xpand.XAF.Modules.JobScheduler.Hangfire.Tests{
    ...
    public abstract class JobSchedulerBaseTest:BlazorBaseTest{
        ...
        protected IObservable<ScheduledJob> MockHangfire(Type testJobType=null,string testName=null) {
            GlobalConfiguration.Configuration.UseMemoryStorage();
            testJobType??=typeof(TestJobDI);
            testName??=nameof(TestJob.Test); 
            
            return ScheduledJobService.CustomJobSchedule.ScheduleImmediate(testJobType.CallExpression(testName));
        }
    }
