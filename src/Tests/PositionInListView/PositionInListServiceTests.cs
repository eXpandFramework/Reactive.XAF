...
namespace Xpand.XAF.Modules.PositionInListView.Tests{
    public class PositionInListServiceTests:PositionInListViewBaseTest{
        ...
        [TestCase(PositionInListViewNewObjectsStrategy.First,-1,-2,-3,-4)][XpandTest()]
        public void When_new_objects_position_them_by_model_strategy(PositionInListViewNewObjectsStrategy? strategy,int first,int second,int third,int fourth){
            using (var applicatin=PositionInListViewModuleModule().Application){
                var modelPositionInListView = applicatin.Model.ToReactiveModule<IModelReactiveModulesPositionInListView>().PositionInListView;
                ...
                if (strategy != null){
                    var modelClassItem = modelPositionInListView.ModelClassItems.AddNode<IModelPositionInListViewModelClassItem>();
        ...
        public void When_listview_creating_sort_collectionsource(SortingDirection sortingDirection,int first,int last){
            using (var applicatin=PositionInListViewModuleModule().Application){
                var positionInListView = applicatin.Model.ToReactiveModule<IModelReactiveModulesPositionInListView>().PositionInListView.ListViewItems.AddNode<IModelPositionInListViewListViewItem>();
