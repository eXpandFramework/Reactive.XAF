	...
	public class ApplyTemplateTests:BaseTests{
		...
		public void When_ApplyTemplateAction_Executed_Applies_active_template_to_all_and_saves_changes_to_source_objects(DocumentStyleLinkOperation operation){
			...
			ApplyTemplate(frame => {
				...
				var oid = objectSpace.GetObjects(modelOffieModule.DefaultPropertiesProvider.TypeInfo.Type, objectSpace.ParseCriteria(modelOffieModule.DefaultPropertiesProviderCriteria)).Cast<DataObject>().First().Oid;
