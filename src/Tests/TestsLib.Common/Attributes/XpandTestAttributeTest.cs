...
namespace Xpand.TestsLib.Common.Attributes{
    public class XpandTestAttributeTest{
        private static int _retryWhenCassetteFailCount;
        ...
        public void Retry_When_TestCase_Fail(int a){
            _retryWhenCassetteFailCount++;
            if (_retryWhenCassetteFailCount < 2 * a){
                throw new Exception(_retryWhenCassetteFailCount.ToString());
            }
        }
    }
}