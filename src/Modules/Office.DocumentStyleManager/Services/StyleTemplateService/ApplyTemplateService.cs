...
namespace Xpand.XAF.Modules.Office.DocumentStyleManager.Services.StyleTemplateService{
	public static class ApplyTemplateService{
		public static string ApplyTemplateStyleActionContainer = nameof(Xpand.XAF.Modules.Office.DocumentStyleManager.BusinessObjects.ApplyTemplateStyle);
		public static SimpleAction ApplyTemplate(this (DocumentStyleManagerModule, Frame frame) tuple) => tuple
			.frame.Action(nameof(ApplyTemplate)).As<SimpleAction>();

		internal static IObservable<Unit> ApplyTemplateStyle(this ApplicationModulesManager manager){
			
			return manager.RegisterApplyTemplesAction().ApplyTemplate().ToUnit()
		...
		private static IObservable<TemplateDocument> ApplyTemplate(this IObservable<SimpleAction> source) => source.WhenExecute()
			.SelectMany(args => {
				var detailView = args.Action.View<DetailView>();
				var changedRichEditControl = detailView.ApplyTemplateStyleChangedRichEditControl();
				var documentsListView = detailView.GetListPropertyEditor<ApplyTemplateStyle>(style => style.Documents).ListView;
				...
				return applyTemplateStyle.Documents
					.Select(document => {
						...
						var templateListViewItem = ((IModelOptionsOfficeModule) detailView.Model.Application.Options).OfficeModule
		...
		internal static ListView ChangedStylesListView(this DetailView detailView) => detailView
			.GetListPropertyEditor<ApplyTemplateStyle>(style => style.ChangedStyles).ListView;

		internal static ListView DocumentsListView(this DetailView detailView) => detailView
			.GetListPropertyEditor<ApplyTemplateStyle>(style => style.Documents).ListView;

		internal static IRichEditDocumentServer ApplyTemplateStyleChangedRichEditControl(this DetailView detailView) => detailView
			.GetPropertyEditor<ApplyTemplateStyle>(o => o.Changed).RichEditControl();

		internal static IRichEditDocumentServer ApplyTemplateOriginalRichEditControl(this DetailView detailView) => detailView
			.GetPropertyEditor<ApplyTemplateStyle>(o => o.Original).RichEditControl();

		private static IObservable<SimpleAction> RegisterApplyTemplesAction(this ApplicationModulesManager manager) => manager
			.RegisterViewSimpleAction(nameof(ApplyTemplate), action => {
