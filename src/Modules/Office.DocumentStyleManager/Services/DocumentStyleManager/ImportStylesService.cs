...
namespace Xpand.XAF.Modules.Office.DocumentStyleManager.Services.DocumentStyleManager{
	public static class ImportStylesService{
		public static SingleChoiceAction FilterImportStyles(this (DocumentStyleManagerModule, Frame frame) tuple) 
            => tuple.frame.Action(nameof(FilterImportStyles)).As<SingleChoiceAction>();

		public static SimpleAction ImportStyles(this (DocumentStyleManagerModule, Frame frame) tuple) 
            => tuple.frame.Action(nameof(ImportStyles)).As<SimpleAction>();

		internal static IObservable<Unit> ImportStyles(this ApplicationModulesManager manager) 
            => manager.ExecuteActions().Merge(manager.ShowStyleListView());
        ...
        private static IObservable<Unit> ShowStyleListView(this ApplicationModulesManager manager)
            => manager.WhenApplication(application => {
                ...
                return viewOnFrame.SelectMany(frame => ((NonPersistentObjectSpace) frame.View.ObjectSpace)
                        .WhenObjectsGetting().SelectMany(t => application.DefaultPropertiesProvider(document => {
        ...
        private static IEnumerable<byte[]> GetBytes(this XafApplication xafApplication,
            ...
            => xafApplication.Model.DocumentStyleManager().ImportStyles
            ...
            private static IObservable<Unit> ExecuteActions(this ApplicationModulesManager manager)
                => manager.ShowStylesListView()
                    .Merge(manager.FilterImportStyles())
                    .Merge(manager.Import());

        private static IObservable<Unit> FilterImportStyles(this ApplicationModulesManager manager)
            => manager.RegisterViewSingleChoiceAction(nameof(FilterImportStyles), action => {
                ...
                .Do(action => {
                    var importStyles = action.Application.Model.DocumentStyleManager().ImportStyles;
                ...
                .TraceDocumentStyleModule(action => action.SelectedItem.Caption)
                ...
                .TraceDocumentStyleModule(action =>action.SelectedItem?.Data?.ToString()??nameof(FilterImportStyles) )
                .ToUnit();
        ...
        private static IObservable<Unit> ShowStylesListView(this ApplicationModulesManager manager)
            => manager.RegisterViewSimpleAction(nameof(ImportStyles), action => {
                ...
                .Do(action => action.Active[nameof(ImportStylesService)] =
                    action.Application.Model.DocumentStyleManager().ImportStyles.Any())
