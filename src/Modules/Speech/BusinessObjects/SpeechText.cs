...
namespace Xpand.XAF.Modules.Speech.BusinessObjects {
    ...
    [CloneModelView(CloneViewType.ListView,SpeechTextKeywordListView)]
    ...
    public class SpeechText:CustomBaseObject,ISelectInExplorer ,IAudioFileLink{
        ...
        public const string SpeechTextKeywordListView = nameof(BusinessObjects.SpeechToText) + "_" +
                                                        nameof(BusinessObjects.SpeechToText.SpeechTexts) + "_Keyword_ListView";
        ...
        public string History => Previous.FromHierarchy(text => text.Previous,text => text!=null).Reverse()
                .Select(text => text.Text.TrimEnd('.')).Join($".{Environment.NewLine}");
        
        [ModelDefault("RowCount","5")]
        public string All => $"{History}{Environment.NewLine}{Text}";
        ...
        // public int Part => (Previous?.SpareTime ?? TimeSpan.Zero) < TimeSpan.FromSeconds(2) ? Previous?.Part ?? 0 : (Previous?.Part ?? 0) + 1;
        ...
        public bool CanConvert =>Next==null|| FileDuration != null && FileDuration.Value != TimeSpan.Zero &&
                                  Duration.Add(SpareTime).Subtract(FileDuration.Value).Add(this.Tolerance()) >= TimeSpan.Zero;
    }
